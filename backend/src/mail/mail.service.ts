...
export class MailService {
  ...
  async sendConfirmation(user: Partial<UserModel>, token: string) {
    const url: string = `${this.configService.get('CLIENT_URL')}/auth/confirm-signup?token=${token}`;
  ...
  async sendResetPassword(user: Partial<UserModel>, token: string) {
    const url: string = `${this.configService.get('CLIENT_URL')}/auth/reset-password?token=${token}`;
  ...
  async sendEmailChange(user: Partial<UserModel>, token: string) {
    const url: string = `${this.configService.get('CLIENT_URL')}/auth/email-change?token=${token}`;
