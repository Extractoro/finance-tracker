...
const UserCategoryItem = ({category, setIsOpen, setMode, setSelected} : ICategoryItemProps) => {
  ...
  return (
    <>
      <DeleteCategoryModal isOpen={isOpenDelete} setIsOpen={setIsOpenDelete} selectedCategory={category}/>
      ...
      >
        ...
        <p
          className="text-sm text-gray-500 opacity-100 transition-opacity duration-200 group-hover:opacity-0">{category.type}</p>

        <div
          className="absolute top-1/2 right-3 -translate-y-1/2 flex gap-2 opacity-0 transition-opacity duration-200 group-hover:opacity-100">
          <button onClick={() => {
            setIsOpen(true);
            setMode('edit')
